...
export class TicketService {
  private allTickets$: BehaviorSubject<HelpTicketWithPagination> = new BehaviorSubject<HelpTicketWithPagination>(null);
  ...
  replies(ticketId: number = 0){
    return this.http.get<any>(`/admin/tickets/${ticketId}`).pipe(tap(res=>{
      const tickets:HelpTicketWithPagination = this.allTickets$.getValue();
      let tkt = find(tickets.data, {id: ticketId});



      tkt.all_children_replies = res;
